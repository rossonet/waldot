 ...
 * under the License.
 */
package net.rossonet.waldot.gremlin.opcgraph.process.traversal.strategy.decoration;
...
public class OptionsStrategyTest {
	...
	private static void assertOptions(final GraphTraversalSource optionedG) {
		...
		t = t.asAdmin().addStep(new ScalarMapStep<Object, Object>(t.asAdmin()) {
			...
			protected Object map(final Traverser.Admin<Object> traverser) {
				final OptionsStrategy strategy = traversal.asAdmin().getStrategies().getStrategy(OptionsStrategy.class)
						.get();
	...
	public void shouldAddOptionsToTraversal() {
		...
		final GraphTraversalSource optionedG = graph.traversal()
				.withStrategies(OptionsStrategy.build().with("a", "test").with("b").create());
