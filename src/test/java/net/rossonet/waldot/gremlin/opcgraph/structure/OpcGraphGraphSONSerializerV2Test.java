...
public class OpcGraphGraphSONSerializerV2Test {
	...
	private final Mapper defaultMapperV2 = GraphSONMapper.build().version(GraphSONVersion.V2_0)
			.addCustomModule(GraphSONXModuleV2.build()).addRegistry(OpcIoRegistryV2.instance()).create();

	private final Mapper noTypesMapperV2 = GraphSONMapper.build().version(GraphSONVersion.V2_0)
			.addCustomModule(GraphSONXModuleV2.build()).typeInfo(TypeInfo.NO_TYPES)
			.addRegistry(OpcIoRegistryV2.instance()).create();

	/**
	 * Checks sequentially vertices and edges of both graphs. Will check
	 * sequentially Vertex IDs, Vertex Properties IDs and values and classes. Then
	 * same for edges. To use when serializing a Graph and deserializing the
	 * supposedly same Graph.
	 */
	private boolean approximateGraphsCheck(Graph g1, Graph g2) {
		...
		while (itV.hasNext()) {
			...
			while (itVP.hasNext()) {
				...
				if (!vp.value().equals(vpRead.value()) || !vp.equals(vpRead)) {
					return false;
				}
			}
		}
		...
		while (itE.hasNext()) {
			...
			while (itEP.hasNext()) {
				...
				if (!ep.value().equals(epRead.value()) || !ep.equals(epRead)) {
					return false;
				}
			}
		}
	...
	public void deserializersTestsEdge() {
		final OpcGraph tg = OpcGraph.open();
		...
		try (final ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			// Anything else would not because we check the type in param here with what's
			// in the JSON, for safety.
	...
	public void deserializersTestsMetrics() {
		...
		final TraversalMetrics tm = tg.traversal().V(1).as("a").has("name").as("b").out("knows").out("created").as("c")
				.has("name", "ripple").values("name").as("d").identity().as("e").profile().next();
		...
		try (final ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			final Metrics metricsRead = (Metrics) reader.readObject(new ByteArrayInputStream(json.getBytes()),
					Object.class);
	...
	public void deserializersTestsPath() {
		...
		final Path p = tg.traversal().V(1).as("a").has("name").as("b").out("knows").out("created").as("c")
				.has("name", "ripple").values("name").as("d").identity().as("e").path().next();
	...
	public void deserializersTestsProperty() {
		...
		try (final ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			final Property pRead = (Property) reader.readObject(new ByteArrayInputStream(json.getBytes()),
					Object.class);
			// can't use equals here, because pRead is detached, its parent element has not
			// been intentionally
	...
	public void deserializersTestsTraversalMetrics() {
		...
		try (final ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			final TraversalMetrics traversalMetricsRead = (TraversalMetrics) reader
					.readObject(new ByteArrayInputStream(json.getBytes()), Object.class);
	...
	public void deserializersTestsVertexProperty() {
		...
		try (final ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			final VertexProperty vPropRead = (VertexProperty) reader
					.readObject(new ByteArrayInputStream(json.getBytes()), Object.class);
	...
	public void shouldDeserializeGraphSONIntoTinkerGraphKeepingTypes() throws IOException {
		...
		v1.addEdge("hello", v2, T.id, 101L, "uuid", UUID.randomUUID());

		try (final ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			writer.writeObject(out, sampleGraph1);
			final String json = out.toString();

			final OpcGraph read = reader.readObject(new ByteArrayInputStream(json.getBytes()), OpcGraph.class);
	...
	public void shouldDeserializeGraphSONIntoTinkerGraphWithoutTypes() throws IOException {
		...
		try (final ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			final String json = out.toString();
			final OpcGraph read = OpcGraph.open();
	...
	/**
	 * Those kinds of types are declared differently in the GraphSON type
	 * deserializer, check that all are handled properly.
	 */
	@Test
	...
	public void shouldLoseTypesWithGraphSONNoTypesForEdgeIds() throws IOException {
		final GraphWriter writer = getWriter(noTypesMapperV2);
		final GraphReader reader = getReader(noTypesMapperV2);
		final OpcGraph sampleGraph1 = OpcGraph.open();
		OpcFactory.generateModern(sampleGraph1);
	...
	public void shouldLoseTypesWithGraphSONNoTypesForEdgeProps() throws IOException {
		...
		final Vertex v1 = sampleGraph1.addVertex(T.id, 100, "name", "kevin");
		v1.addEdge("hello", sampleGraph1.traversal().V().has("name", "marko").next(), T.id, 101, "uuid",
				UUID.randomUUID());
		try (final ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			writer.writeGraph(out, sampleGraph1);
			final String json = out.toString();
	...
	/**
	 * Asserts the approximateGraphsChecks function fails when expected. Vertex
	 * props.
	 */
	@Test
