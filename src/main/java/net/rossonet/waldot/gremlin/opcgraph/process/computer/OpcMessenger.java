 ...
 */
public final class OpcMessenger<M> implements Messenger<M> {

    private final Vertex vertex;
    private final OpcMessageBoard<M> messageBoard;
    private final MessageCombiner<M> combiner;

    public OpcMessenger(final Vertex vertex, final OpcMessageBoard<M> messageBoard, final Optional<MessageCombiner<M>> combiner) {
        this.vertex = vertex;
...
//        for (final MessageScope messageScope : this.messageBoard.previousMessageScopes) {
            if (messageScope instanceof MessageScope.Local) {
                ...
                final Traversal.Admin<Vertex, Edge> incidentTraversal = OpcMessenger.setVertexStart(localMessageScope.getIncidentTraversal().get().asAdmin(), this.vertex);
                final Direction direction = OpcMessenger.getDirection(incidentTraversal);
