...
public class MiloSingleServerBaseV0Strategy implements OpcMappingStrategy {
	...
	@Override
	public Edge addEdge(OpcVertex sourceVertex, OpcVertex targetVertex, String label, Object[] propertyKeyValues) {
		...
		for (int i = 0; i < propertyKeyValues.length; i = i + 2) {
			if (propertyKeyValues[i] instanceof String && propertyKeyValues[i] != null) {
				createOrUpdateOpcEdgeProperty(edge, (String) propertyKeyValues[i], propertyKeyValues[i + 1]);
			}
		}
		return edge;
	}
	...
	public OpcVertex addVertex(NodeId nodeId, Object[] propertyKeyValues) {
		// logger.info("addVertex: nodeId={}, propertyKeyValues={}", nodeId,
		// propertyKeyValues);
		...
		if (typeDefinition.equals(ruleTypeNode.getNodeId())) {
			final String directory = getDirectory(propertyKeyValues);
			if (directory == null || directory.isEmpty()) {
				rulesFolderNode.addOrganizes(vertex);
			} else {
				...
				rulesDirectories.get(directory).addOrganizes(vertex);
			}
			...
			vertex.setProperty(ACTION, action);
			if (condition == null) {
			...
			vertex.setProperty(CONDITION, condition);
			if (priority == null) {
			...
			vertex.setProperty(PRIORITY, priority);
			final QualifiedProperty<String> LABEL = new QualifiedProperty<String>(waldotNamespace.getNamespaceUri(),
					LABEL_FIELD, labelRuleTypeNode.getNodeId().expanded(), ValueRanks.Scalar, String.class);
			vertex.setProperty(LABEL, label);
			waldotNamespace.getRulesEngine().registerOrUpdate(vertex.getNodeId(), label, condition, action, priority);
		} else {
			final String directory = getDirectory(propertyKeyValues);
			if (directory == null || directory.isEmpty()) {
		...
		for (int i = 0; i < propertyKeyValues.length; i = i + 2) {
			if (propertyKeyValues[i] instanceof String && propertyKeyValues[i] != null) {
				createOrUpdateOpcVertexProperty(vertex, (String) propertyKeyValues[i], propertyKeyValues[i + 1]);
			}
		}
		return vertex;
	}
	...
	public <DATA_TYPE> OpcProperty<DATA_TYPE> createOrUpdateOpcEdgeProperty(OpcEdge opcEdge, String key,
			DATA_TYPE value) {
		// logger.info("createOpcEdgeProperty: key={}, value={}", key, value);
	...
	@SuppressWarnings("unchecked")
	@Override
	public <DATA_TYPE> OpcVertexProperty<DATA_TYPE> createOrUpdateOpcVertexProperty(OpcVertex opcVertex, String key,
			DATA_TYPE value) {
		// logger.info("createOrUpdateOpcVertexProperty: key={}, value={}", key, value);
		final NodeId nodeId = waldotNamespace
	...
	public OpcVertex getEdgeInVertex(OpcEdge opcEdge) {
		// logger.info("getEdgeInVertex: opcEdge={}", opcEdge);
		final QualifiedProperty<NodeId> TARGET = new QualifiedProperty<NodeId>(waldotNamespace.getNamespaceUri(),
				"TargetNode", targetNodeTypeNode.getNodeId().expanded(), ValueRanks.Scalar, NodeId.class);
		final NodeId nodeId = opcEdge.getProperty(TARGET).get();
		return (OpcVertex) waldotNamespace.getStorageManager().getNode(nodeId).get();
	}

	@Override
	public OpcVertex getEdgeOutVertex(OpcEdge opcEdge) {
		// logger.info("getEdgeOutVertex: opcEdge={}", opcEdge);
		final QualifiedProperty<NodeId> SOURCE = new QualifiedProperty<NodeId>(waldotNamespace.getNamespaceUri(),
				"SourceNode", sourceNodeTypeNode.getNodeId().expanded(), ValueRanks.Scalar, NodeId.class);
		final NodeId nodeId = opcEdge.getProperty(SOURCE).get();
		return (OpcVertex) waldotNamespace.getStorageManager().getNode(nodeId).get();
	}
	...
	public <DATA_TYPE> List<OpcProperty<DATA_TYPE>> getEdgeProperties(OpcEdge opcEdge) {
		// logger.info("getEdgeProperties: opcEdge={}", opcEdge);
		final List<OpcProperty<DATA_TYPE>> result = new ArrayList<>();
		for (final Reference p : opcEdge.getReferences()) {
			if (p.isForward() && p.getReferenceTypeId().equals(hasPropertyReferenceType)) {
				result.add((OpcProperty<DATA_TYPE>) waldotNamespace.getStorageManager()
	...
	public Map<NodeId, Edge> getEdges() {
		// logger.info("getEdges");
		final Map<NodeId, Edge> result = new HashMap<>();
		for (final Node e : edgesFolderNode.getOrganizesNodes()) {
			if (e instanceof Edge) {
				result.put(e.getNodeId(), (Edge) e);
			}
		}
	...
	public <DATA_TYPE> OpcEdge getPropertyReference(OpcProperty<DATA_TYPE> opcProperty) {
		// logger.info("getPropertyReference: opcProperty={}", opcProperty);
		for (final Reference r : opcProperty.getReferences()) {
	...
	public <DATA_TYPE> Map<String, OpcVertexProperty<DATA_TYPE>> getVertexProperties(OpcVertex opcVertex) {
		// logger.info("getVertexProperties: opcVertex={}", opcVertex);
		final Map<String, OpcVertexProperty<DATA_TYPE>> result = new HashMap<>();
		for (final Reference p : opcVertex.getReferences()) {
			if (p.isForward() && p.getReferenceTypeId().equals(hasPropertyReferenceType)) {
				final UaNode node = waldotNamespace.getStorageManager()
						.getNode(p.getTargetNodeId().toNodeId(waldotNamespace.getNamespaceTable()).get()).get();
				result.put(node.getBrowseName().getName(), (OpcVertexProperty<DATA_TYPE>) node);
			}
		}
		return result;
	}
	...
	public <DATA_TYPE> OpcVertex getVertexPropertyReference(OpcVertexProperty<DATA_TYPE> opcVertexProperty) {
		// logger.info("getVertexPropertyReference: opcVertexProperty={}",
		// opcVertexProperty);
	...
	public Map<NodeId, Vertex> getVertices() {
		// logger.info("getVertices");
		final Map<NodeId, Vertex> result = new HashMap<>();
		for (final Node v : verticesFolderNode.getOrganizesNodes()) {
			if (v instanceof Vertex) {
				result.put(v.getNodeId(), (Vertex) v);
			}
		}
	...
	@Override
	public HomunculusNamespace initialize(HomunculusNamespace waldotNamespace) {
		// logger.info("initialize: waldotNamespace={}", waldotNamespace);
	...
	public Object namespaceParametersGet(String key) {
		// logger.info("namespaceParametersGet: key={}", key);
	...
	public Set<String> namespaceParametersKeySet() {
		// logger.info("namespaceParametersKeySet");
	...
	public void namespaceParametersPut(String key, Object value) {
		// logger.info("namespaceParametersPut: key={}, value={}", key, value);
		if (waldotNamespace.hasNodeId(getParameterNodeId(key))) {
			final UaVariableNode property = (UaVariableNode) waldotNamespace.getStorageManager()
					.getNode(getParameterNodeId(key)).get();
	...
	public void namespaceParametersRemove(String key) {
		// logger.info("namespaceParametersRemove: key={}", key);
		if (waldotNamespace.hasNodeId(getParameterNodeId(key))) {
			final UaVariableNode property = (UaVariableNode) waldotNamespace.getStorageManager()
					.getNode(getParameterNodeId(key)).get();
	...
	public void registerCommand(WaldotCommand command) {
		waldotNamespace.getStorageManager().addNode((AbstractWaldotCommand) command);
		interfaceRootNode.addComponent((AbstractWaldotCommand) command);

	}

	@Override
	public void registerCommandInputArgument(AbstractWaldotCommand waldotCommand, List<Argument> inputArguments) {
		waldotCommand.setInputArguments(inputArguments.toArray(new Argument[0]));

	}

	@Override
	public void registerCommandOutputArguments(AbstractWaldotCommand waldotCommand, List<Argument> outputArguments) {
	...
	public void removeCommand(WaldotCommand command) {
		interfaceRootNode.removeComponent((AbstractWaldotCommand) command);
		waldotNamespace.getStorageManager().removeNode(command.getNodeId());
	}

	@Override
	public void removeEdge(NodeId nodeId) {
		// logger.info("removeEdge: nodeId={}", nodeId);
	...
	public void removeVertex(NodeId nodeId) {
		// logger.info("removeVertex: nodeId={}", nodeId);
		final UaNode node = waldotNamespace.getStorageManager().getNode(nodeId).get();
		// FIXME rimuovere dalla cartella giusta
		waldotNamespace.getStorageManager().removeNode(nodeId);
		waldotNamespace.getRulesEngine().deregister(node.getNodeId());
		node.delete();
	...
	public void removeVertexProperty(NodeId nodeId) {
		// logger.info("removeVertexProperty: nodeId={}", nodeId);
		final UaNode node = waldotNamespace.getStorageManager().getNode(nodeId).get();
		waldotNamespace.getStorageManager().removeNode(nodeId);
		node.delete();
		// FIXME rimuovere tutti i nodi OpcProperty collegati
