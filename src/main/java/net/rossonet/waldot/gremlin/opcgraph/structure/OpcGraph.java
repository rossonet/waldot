...
public class OpcGraph extends AbstractOpcGraph {
	...
	private HomunculusNamespace opcNamespace;
	...
	public Vertex addVertex(final Object... keyValues) {
		...
		final OpcVertex vertex = opcNamespace.addVertex(nodeId, keyValues);
		return vertex;
	}
	...
	protected <T extends Element> Iterator<T> createElementIterator(final Class<T> clazz, final Map<NodeId, T> elements,
			final IdManager<?> idManager, final NodeId[] ids) {
		final Iterator<T> iterator;
		if (0 == ids.length) {
			iterator = new OpcGraphIterator<>(elements.values().iterator());
		} else {
			final List<Object> idList = Arrays.asList(ids);

			return new OpcGraphIterator<>(IteratorUtils.filter(IteratorUtils.map(idList, id -> {

				if (null == id) {
					return null;
				}
				final Object iid = clazz.isAssignableFrom(id.getClass()) ? clazz.cast(id).id()
						: idManager.convert(this, id);
				return elements.get(idManager.convert(this, iid));
			}).iterator(), Objects::nonNull));
		}
		return (getOpcNamespace().inComputerMode()) ? (Iterator<T>) (clazz.equals(Vertex.class)
	...
	@Override
	public HomunculusNamespace getOpcNamespace() {
		return opcNamespace;
	}
	...
	@Override
	public void setNamespace(HomunculusNamespace waldotNamespace) {
