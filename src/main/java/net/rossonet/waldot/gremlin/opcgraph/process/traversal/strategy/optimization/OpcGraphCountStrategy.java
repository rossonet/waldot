 ...
 * @example <pre>
 * g.V().count()               // is replaced by OpcCountGlobalStep
 * g.V().map(out()).count()    // is replaced by OpcCountGlobalStep
 * g.E().label().count()       // is replaced by OpcCountGlobalStep
 * </pre>
 */
public final class OpcGraphCountStrategy extends AbstractTraversalStrategy<TraversalStrategy.ProviderOptimizationStrategy> implements TraversalStrategy.ProviderOptimizationStrategy {

    private static final OpcGraphCountStrategy INSTANCE = new OpcGraphCountStrategy();

    private OpcGraphCountStrategy() {
    }
    ...
    public void apply(final Traversal.Admin<?, ?> traversal) {
        ...
        traversal.addStep(new OpcCountGlobalStep<>(traversal, elementClass));
    }

    @Override
    public Set<Class<? extends ProviderOptimizationStrategy>> applyPost() {
        return Collections.singleton(OpcGraphStepStrategy.class);
    }

    public static OpcGraphCountStrategy instance() {
        return INSTANCE;
    }
}
