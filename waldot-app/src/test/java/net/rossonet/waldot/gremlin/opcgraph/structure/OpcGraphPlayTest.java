 ...
 */
public class OpcGraphPlayTest {
	private static final Logger logger = LoggerFactory.getLogger(OpcGraphPlayTest.class);
	...
	public void benchmarkStandardTraversals() throws Exception {
		...
		final List<Supplier<Traversal>> traversals = Arrays.asList(() -> g.V().outE().inV().outE().inV().outE().inV(),
				() -> g.V().out().out().out(), () -> g.V().out().out().out().path(), () -> g.V().repeat(out()).times(2),
				() -> g.V().repeat(out()).times(3), () -> g.V().local(out().out().values("name").fold()),
	...
	public void testBugs() throws InterruptedException, ExecutionException {
		...
		final Traversal<?, ?> t = g.V("3").union(__.repeat(out().simplePath()).times(2).count(),
	...
	public void testPaths() throws Exception {
		...
		graph.io(GraphMLIo.build()).readGraph(
				"/Users/twilmes/work/repos/scratch/tinkerpop/gremlin-test/src/main/resources/org/apache/tinkerpop/gremlin/structure/io/graphml/grateful-dead.xml");
//        graph = OpcFactory.createModern();
		final GraphTraversalSource g = graph.traversal().withComputer(Computer.compute().workers(1));

		System.out.println(g.V()
				.match(as("a").in("sungBy").as("b"), as("a").in("sungBy").as("c"), as("b").out("writtenBy").as("d"),
						as("c").out("writtenBy").as("e"), as("d").has("name", "George_Harrison"),
...
//                __.as("b").out().as("c")).select("c").count().profile().next());
	...
	public void testPlay4() throws Exception {
		...
		final List<Supplier<Traversal>> traversals = Arrays.asList(
				() -> g.V().has(T.label, "song").out().groupCount()
						.<Vertex>by(t -> g.V(t)
								.choose(r -> g.V(r).has(T.label, "artist").hasNext(), in("writtenBy", "sungBy"),
										both("followedBy"))
								.values("name").next())
						.fold(),
				() -> g.V().has(T.label, "song").out().groupCount()
						.<Vertex>by(t -> g.V(t)
								.choose(has(T.label, "artist"), in("writtenBy", "sungBy"), both("followedBy"))
								.values("name").next())
						.fold(),
				() -> g.V().has(T.label, "song").out().groupCount()
						.by(choose(has(T.label, "artist"), in("writtenBy", "sungBy"), both("followedBy"))
								.values("name"))
						.fold(),
				() -> g.V().has(T.label, "song").both().groupCount()
						.<Vertex>by(t -> g.V(t).both().values("name").next()),
	...
	public void testPlay5() throws Exception {
		...
		final Supplier<Traversal<?, ?>> traversal = () -> g.V()
				.match(as("a").has("name", "Garcia"), as("a").in("writtenBy").as("b"),
						as("b").out("followedBy").as("c"), as("c").out("writtenBy").as("d"), as("d").where(neq("a")))
				.select("a", "b", "c", "d").by("name");
	...
	public void testPlay6() throws Exception {
		...
		logger.info(TimeUtil
				.clockWithResult(500,
						() -> g.V().match(as("a").out("knows").as("b"), as("a").out("uncle").as("b")).toList())
				.toString());
	}
	...
	public void testPlay7() throws Exception {
		/*
		 * OpcGraph graph = OpcGraph.open(); graph.createIndex("name",Vertex.class);
		 * graph.io(GraphMLIo.build()).readGraph(
		...
		final List<Supplier<GraphTraversal<?, ?>>> traversals = Arrays.asList(() -> g.V().out().as("v")
				...
				.local(union(select("v"), select("inDegree", "outDegree")).unfold().fold()));
	...
	public void testPlay8() throws Exception {
		...
		final GraphTraversalSource g = graph.traversal();
		System.out.println(g.withSack(1).inject(1).repeat(sack((BiFunction) sum).by(constant(1))).times(10).emit()
				.math("sin _").by(sack()).toList());
	}
	...
	public void testPlay9() throws Exception {
		...
		final GraphTraversalSource g = graph.traversal().withComputer(Computer.compute().workers(4))
				.withStrategies(PathRetractionStrategy.instance());
		final GraphTraversalSource h = graph.traversal().withComputer(Computer.compute().workers(4))
				.withoutStrategies(PathRetractionStrategy.class);
	...
	public void WithinTest() {
		...
		final List result = g.V().has(T.id, P.lt(count / 2)).aggregate("v1").V().where(P.without("v1")).toList();
