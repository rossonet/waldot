/**
    ...
    */
package net.rossonet.oshi.gremlin;
...
public final class OshiGremlinHelper {
	private final static Logger logger = LoggerFactory.getLogger(OshiGremlinHelper.class);
	...
	public static List<OshiMethodWrapper> elaborateInstance(final WaldotGraph g, final Object analizedObject,
		...
		if (analizedObject instanceof List) {
			...
			queryData.add("label");
			queryData.add(analizedObject.getClass().getSimpleName());
			queryData.add("id");
			queryData.add(parentName + "/" + analizedObject.getClass().getSimpleName());
			if (parentName != null) {
				...
				try {
					if (checkValidMethod(parentName, name)) {
						...
						if (method.getReturnType().isPrimitive() || method.getReturnType().isEnum()
							...
							// TODO: add update trigger for primitive values
							// final OshiMethodWrapper currentTrigger = new OshiMethodWrapper(v,
							// analizedObject, method);
