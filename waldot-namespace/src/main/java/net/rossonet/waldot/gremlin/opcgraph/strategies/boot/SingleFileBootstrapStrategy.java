...
public class SingleFileBootstrapStrategy implements BootstrapStrategy {
	 ...
	 * 
	 * <p>This method supports two configuration formats for maximum flexibility:</p>
	 * 
	 * <h3>Format 1: Line-by-line execution (legacy, backward compatible)</h3>
	 * <p>Each non-comment, non-empty line is executed as a separate Groovy expression.
	 * Lines starting with '#' are treated as comments and skipped.</p>
	 * 
	 * <pre>{@code
	 * # This is a comment
	 ...
	 * <p>If the file contains multi-line constructs (variable assignments, functions, 
	 * control structures), the entire content is treated as a single Groovy script.
	 * Comments with '//' or '/*' are preserved for the Groovy parser.</p>
	 * 
	 * <pre>{@code
	 ...
	 * }</pre>
	 * 
	 * <p>The method automatically detects which format to use by analyzing the content:
	 * <ul>
	 *   <li>If it finds variable assignments (=), function definitions (def), 
	 *       or multi-line constructs, it uses script mode</li>
	 *   <li>Otherwise, it uses line-by-line mode for backward compatibility</li>
	 * </ul>
	 * 
	 * <p><b>Remote URLs</b>: The configuration can be loaded from:
	 * <ul>
	 ...
	 * @return Array of configuration strings to execute. In script mode, returns
	 *         a single-element array with the entire script. In line mode, returns
	 *         an array with one expression per element.
	 */
	private String[] getConfigurationfromUrl(final String bootstrapUrl) {
		try {
			...
			boolean useScriptMode = detectScriptMode(content);
	...
	/**
	 * Detects if the configuration should be executed as a complete script
	 * or line-by-line.
	 * 
	 * <p>Script mode is used when the content contains:</p>
	 * <ul>
	 ...
	 * </ul>
	 * 
	 * <p>Line-by-line mode is used for simple configurations with one
	 * command per line and hash comments (legacy format).</p>
	 * 
	...
	private boolean detectScriptMode(final String content) {
		...
		if (java.util.regex.Pattern.compile("^\\s*\\w+\\s*=(?!=)", java.util.regex.Pattern.MULTILINE).matcher(cleanContent).find()) {
			return true;
		}
