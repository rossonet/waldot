...
package net.rossonet.waldot.rules.btree;
...
/**
 * This is the abstract base class of all behavior tree tasks. The {@code Task}
 * of a behavior tree has a status, one control and a list of children.
 * 
...
public abstract class Task<E> implements Poolable {
	...
	/**
	 * The clone strategy (if any) that {@link #cloneTask()} will use. Defaults to
	 * {@code null}, meaning that {@link #copyTo(Task)} is used instead. In this
	 * case, properly overriding this method in each task is developer's
	 * responsibility but this gives you the opportunity to target GWT.
	 * <p>
	 * For instance, if you don't care about GWT, you can let Kryo make a deep copy
	 * for you like that
	 * 
	 * <pre>
	 * <code>
	 ...
	 * @return {@code true} if guard evaluation succeeds or there's no guard;
	 *         {@code false} otherwise.
	 * @throws IllegalStateException if guard evaluation returns any status other
	 *                               than {@link Status#SUCCEEDED} and
	 *                               {@link Status#FAILED}.
	 */
	 ...
	 * {@link #TASK_CLONER} the new task is instantiated via reflection and
	 * {@link #copyTo(Task)} is invoked.
	 * 
	...
	public Task<E> cloneTask() {
		...
		try {
			// TODO da capire come implementare senza kryo
			// Task<E> clone = copyTo(ClassReflection.newInstance(this.getClass()));
			Task<E> clone = new Task<>() {
	 ...
	 * {@link #cloneTask()} only if {@link #TASK_CLONER} is {@code null} which is
	 * its default value.
	 * 
