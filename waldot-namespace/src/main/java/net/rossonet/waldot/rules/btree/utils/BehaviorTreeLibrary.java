...
import net.rossonet.waldot.rules.btree.TaskCloneException;

/**
 * A {@code BehaviorTreeLibrary} is a repository of behavior tree archetypes.
 * Behavior tree archetypes never run. Indeed, they are only cloned to create
 * behavior tree instances that can run.
 * 
 * @author davebaol
 */
public class BehaviorTreeLibrary {
	...
	/**
	 * Creates a {@code BehaviorTreeLibrary} using the new internal resolver
	 * returned by the call {@link FileSystem#newResolver(FileType)
...
//		this.assetManager = assetManager;
	...
	/**
	 * Creates a {@code BehaviorTreeLibrary} with the given resolver and debug
	 * level.
	 * 
	 * @param resolver        the {@link FileHandleResolver}
...
//	}
	...
	/**
	 * Creates a {@code BehaviorTreeLibrary} with the given debug level and using
	 * the new internal resolver returned by the call
	 * {@link FileSystem#newResolver(FileType)
	 * GdxAI.getFileSystem().newResolver(FileType.Internal)}.
	 * 
	 * @param parseDebugLevel the debug level the parser will use
	 */
	 ...
	 * @throws SerializationException if the reference cannot be successfully
	 *                                parsed.
	 * @throws TaskCloneException     if the archetype cannot be successfully
	 *                                parsed.
	 */
	...
	/**
	 * Creates the {@link BehaviorTree} for the specified reference and blackboard
	 * object.
	 * 
	 * @param treeReference the tree identifier, typically a path
	...
	/**
	 * Returns {@code true} if an archetype tree with the specified reference is
	 * registered in this library.
	 * 
	 * @param treeReference the tree identifier, typically a path.
	 * @return {@code true} if the archetype is registered already; {@code false}
	 *         otherwise.
	 */
	...
	/**
	 * Registers the {@link BehaviorTree} archetypeTree with the specified
	 * reference. Existing archetypes in the repository with the same treeReference
	 * will be replaced.
	 * 
	 * @param treeReference the tree identifier, typically a path.
	...
	/**
	 * Retrieves the archetype tree from the library. If the library doesn't contain
	 * the archetype tree it is loaded and added to the library.
	 * 
	 * @param treeReference the tree identifier, typically a path
