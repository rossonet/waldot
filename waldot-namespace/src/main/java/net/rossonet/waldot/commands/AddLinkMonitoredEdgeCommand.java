...
public class AddLinkMonitoredEdgeCommand extends AbstractOpcCommand {
	private static final String CONTEXT_DIRECTORY = MiloStrategy.MANAGE_CMD_DIRECTORY
			+ "/edges";
	private static final String CREATE_LINK_MONITORED_EDGE_DESCRIPTION = "create a monitored edge data link";
	private static final String CREATE_LINK_MONITORED_EDGE_NAME = "create link monitored edge";
	...
	protected final static Logger logger = LoggerFactory
			.getLogger(AddLinkMonitoredEdgeCommand.class);
	private static final String LONG_LABEL_RESULT = "result of the update operation";
	public AddLinkMonitoredEdgeCommand(final WaldotNamespace waldotNamespace) {
		...
		addInputArgument(MonitoredEdge.PROPERTY_EDGE_INVERTED_LABEL,
				VariableNodeTypes.Boolean.getNodeId(), ValueRanks.Scalar, null,
				LocalizedText.english(
						MonitoredEdge.PROPERTY_EDGE_INVERTED_DESCRIPTION));
	...
	@Override
	public Object clone() {
		return new AddLinkMonitoredEdgeCommand(this.waldotNamespace);
	}
	...
	public String[] runCommand(final InvocationContext invocationContext,
		...
		if (inputValues[4] != null && !inputValues[6].isEmpty()
				&& Boolean.parseBoolean(inputValues[4]) == true) {
			parameters.add(MiloStrategy.LINK_FROM_EDGE_TYPE);
		} else {
			parameters.add(MiloStrategy.LINK_TO_EDGE_TYPE);
		}
