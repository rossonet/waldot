 ...
 */
final class GraphSONTranslator<S extends TraversalSource, T extends Traversal.Admin<?, ?>>
	...
	public GraphSONTranslator(final JavaTranslator<S, T> wrappedTranslator, final GraphSONVersion version) {
		...
		if (version == GraphSONVersion.V2_0) {
			mapper = GraphSONMapper.build().addCustomModule(GraphSONXModuleV2.build()).version(GraphSONVersion.V2_0)
					.create();
		} else if (version == GraphSONVersion.V3_0) {
			mapper = GraphSONMapper.build().addCustomModule(GraphSONXModuleV3.build()).version(GraphSONVersion.V3_0)
					.create();
		} else {
	...
	public T translate(final Bytecode bytecode) {
		try {
			...
			return this.wrappedTranslator.translate(
					this.reader.readObject(new ByteArrayInputStream(outputStream.toByteArray()), Bytecode.class));
