...
//TODO inserire assert nei test
	...
	public void runAllGeneratorServer() throws Exception {
		...
		for (int i = 0; i < 5000; i++) {
			...
			g.addVertex(MiloStrategy.DIRECTORY_PARAMETER, "incrmental", MiloStrategy.ID_PARAMETER, "incrmental" + i,
					WaldotGeneratorPlugin.ALGORITHM_FIELD.toLowerCase(), "incrmental",
					MiloStrategy.LABEL_FIELD.toLowerCase(), "incrmental" + i, MiloStrategy.TYPE_DEFINITION_PARAMETER,
					WaldotGeneratorPlugin.DATA_GENERATOR_OBJECT_TYPE_LABEL,
					WaldotGeneratorPlugin.DELAY_FIELD.toLowerCase(), "1000");
	...
	public void runSimpleGeneratorServer() throws Exception {
		...
		g.addVertex(MiloStrategy.ID_PARAMETER, "test3", MiloStrategy.LABEL_FIELD.toLowerCase(), "test3",
				MiloStrategy.TYPE_DEFINITION_PARAMETER, WaldotGeneratorPlugin.DATA_GENERATOR_OBJECT_TYPE_LABEL,
				WaldotGeneratorPlugin.DELAY_FIELD.toLowerCase(), "400",
				WaldotGeneratorPlugin.ALGORITHM_FIELD.toLowerCase(), "decremental",
		...
		g.addVertex(MiloStrategy.ID_PARAMETER, "test4", MiloStrategy.LABEL_FIELD.toLowerCase(), "test4",
				MiloStrategy.TYPE_DEFINITION_PARAMETER, WaldotGeneratorPlugin.DATA_GENERATOR_OBJECT_TYPE_LABEL,
				WaldotGeneratorPlugin.DELAY_FIELD.toLowerCase(), "4000",
				WaldotGeneratorPlugin.ALGORITHM_FIELD.toLowerCase(), "random",
				WaldotGeneratorPlugin.MAX_VALUE_FIELD.toLowerCase(), "-1",
		...
		g.addVertex(MiloStrategy.ID_PARAMETER, "test5", MiloStrategy.LABEL_FIELD.toLowerCase(), "test5",
				MiloStrategy.TYPE_DEFINITION_PARAMETER, WaldotGeneratorPlugin.DATA_GENERATOR_OBJECT_TYPE_LABEL,
				WaldotGeneratorPlugin.DELAY_FIELD.toLowerCase(), "4000",
				WaldotGeneratorPlugin.ALGORITHM_FIELD.toLowerCase(), "error_field");
		Thread.sleep(120_000);
		g.getWaldotNamespace().getOpcuaServer().close();
	}
}
